...
#[repr(transparent)]
pub struct AtomicChain<K, V> {
    ptr: *const AtomicChainInner<K, V>,
}

unsafe impl<K: Send, V: Send> Send for AtomicChain<K, V> {}
unsafe impl<K: Send, V: Send> Sync for AtomicChain<K, V> {}

impl<K, V> UnwindSafe for AtomicChain<K, V> {}
impl<K, V> RefUnwindSafe for AtomicChain<K, V> {}

impl<K: Eq + Hash, V> AtomicChain<K, V> {
    pub fn new() -> Self {
    ...
    #[inline(always)]
    fn inner(&self) -> &AtomicChainInner<K, V> {
        unsafe { &*self.ptr }
    }
    ...
    /// Inserisce un valore mantenendo FIFO per chiave
    pub fn insert(&self, key: K, value: V) {
        ...
        let mut cur = head;
        let mut last_same_key: *mut Item<K, V> = null_mut();
        ...
        unsafe {
            if !last_same_key.is_null() {
                (*last_same_key).next.store(new_item, Ordering::Release);
            } else {
                (*new_item).next.store(head, Ordering::Release);
    ...
    ///Return all the values associated to the key
    pub fn get_all<Q: ?Sized>(&self, key: &Q) -> Vec<WatchGuardRef<'_, V>>
    where
        K: Borrow<Q>,
        Q: Hash + Eq,
    {
        ...
        while !cur.is_null() {
            unsafe {
                if (*cur).key.borrow() == key {
                    ...
                    bucket.ref_locked.unlock_group();
                }
                cur = (*cur).next.load(Ordering::Acquire);
            }
        }
...
impl<K, V> Clone for AtomicChain<K, V> {
    fn clone(&self) -> Self {
...
impl<K, V> Drop for AtomicChain<K, V> {
    fn drop(&mut self) {
        let inner = unsafe { &*self.ptr };
        if inner.ref_count.fetch_sub(1, Ordering::Release) == 1 {
            ...
            unsafe { drop(Box::from_raw(self.ptr as *mut AtomicChainInner<K, V>)) };
        }
    }
}

impl<K, V> fmt::Debug for AtomicChain<K, V> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let inner = unsafe { &*self.ptr };
        f.debug_struct("AtomicChain")
            .field("len", &inner.len)
...
pub struct Iter<'a, K, V> {
    map: &'a AtomicChain<K, V>,
    bucket_idx: usize,
    current: *mut Item<K, V>,
}

impl<'a, K: Eq + Hash, V> Iter<'a, K, V> {
    fn new(map: &'a AtomicChain<K, V>) -> Self {
        let mut it = Iter {
            map,
            bucket_idx: 0,
            current: null_mut(),
